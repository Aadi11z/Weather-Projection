...
\subsection{Why the Fuzzy Correction Matters for Inference}

A key insight is that the fuzzy post-processor does more than improve
the physical realism of individual predictions ,  it materially changes
the \emph{statistical inference} performed downstream.  Without the fuzzy
correction,
\begin{enumerate}[leftmargin=1.5em]
  \item the CNN-LSTM output is effectively a spatially smooth climate
    field with no urban-rural differentiation, because ERA5 itself
    under-represents sub-grid urban heterogeneity;
  \item the MK test still detects a real warming trend but
    \emph{underestimates} its amplitude at the urban core, potentially
    missing significant pixels in the suburban transition zone;
  \item the SQ-MK mutation analysis identifies the same structural
    change years but with smaller \(|u(k)|\), sometimes failing to exceed
    the \(\alpha=0.05\) threshold altogether.
\end{enumerate}
With the fuzzy correction, the urban core exhibits systematically higher
warming amplitudes, producing larger \(|Z|\) values and earlier mutation
detection ,  a physically meaningful difference with direct implications
for urban heat adaptation planning and infrastructure design
\cite{santamouris2013urban}.

\subsection{Interpretability vs.\ End-to-End Learning}

A common alternative would be to fold the urban correction into the
CNN-LSTM itself, e.g.\ via an attention module conditioned on a
land-use mask.  The deliberate choice of a separate fuzzy stage brings
three advantages that a black-box end-to-end network cannot match:
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Editability.}  A domain expert can rewrite a rule, nudge
    a membership support, or disable a whole FIS without retraining the
    network.
  \item \textbf{Auditability.}  Every adjustment can be traced back to a
    concrete firing pattern (Eq.~\ref{eq:firing}) and defuzzified scalar
    (Eq.~\ref{eq:centroid}), satisfying the kind of review demanded in
    climate-policy contexts.
  \item \textbf{Safety.}  Hard excluded channels (PCP, AP) cannot
    accidentally drift as they might in a network trained to minimise
    only a composite loss.
\end{itemize}

\subsection{Computational Economics}

The LUT-based implementation enables a throughput on the order of
\(10^4\) grids per second on a single CPU thread (cf.\ the empirical
benchmark in \texttt{fuzzy\_uhi.py}).  Processing the
31{,}400-day historical corpus costs seconds rather than hours; a
multi-ensemble 86-year SSP run costs minutes.  This reduces the
operational cost of running the correction inside a hyper-parameter
search or ensemble loop to effectively zero compared to the CNN-LSTM's
own inference cost.
